        ...
        if line.startswith("Row"):
            if row:  # Save previous row
                ...
                if "Priority" not in row or row["Priority"] not in ["Yes", "No"]:
                    row["Priority"] = "No"  # Default to "No" if missing or invalid
                data.append(row)
            row = {}
            analysis_lines = []
            remediation_lines = []
...
print(df["Priority"].tolist())
print("Any NaN in Priority before mapping?", df["Priority"].isna().any())

# Map Severity to numeric values
severity_map = {"Low": 1, "Moderate": 2, "High": 3, "Critical": 4}
df["Severity"] = df["Severity"].map(severity_map)
df = df.dropna(subset=["Severity", "Priority"])  # Just in case

# Debug: After dropna
print("After dropna - Priority values:")
print(df["Priority"].tolist())
...
joblib.dump(model, os.path.join(model_dir, "lr_model_20.pkl"))
joblib.dump(tfidf, os.path.join(model_dir, "tfidf_vectorizer_20.pkl"))
